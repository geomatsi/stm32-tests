...
#define dmesg(format, args...) \
	do { \
		memset(msg, 0x0, sizeof(msg)); \
		/* FIXME: potential buffer overflow - snprintf needed */ \
		sprintf(msg, format, ## args); \
		int len = strlen(msg); \
		if (len) { \
			while (usbd_ep_write_packet(g_usbd_dev, 0x82, msg, len) == 0); \
		} \
	} while(0)
...
static void cdcacm_data_rx_cb(usbd_device *usbd_dev, uint8_t ep)
{
	...
	//while (usbd_ep_write_packet(usbd_dev, 0x82, rsp, strlen(rsp)) == 0);
}
